...
{
    public static Guid FixedID = new Guid("82ad86a3-09f9-4e85-a3f0-bd68e3027299");
...
#region fixtures
    public static Domain.Aggregates.Order.Order Create_Order_Required_Properties_OK(DpTest dpTest)
    {
        ...
        dpTest.Set<string>(order, "PromoCode", Faker.Lorem.Sentence(1));
        return order;
    }
    ...
    public static Domain.Aggregates.Order.Order Create_Order_With_PromoCode_Required_Property_Missing(DpTest dpTest)
    {
        var order = new Domain.Aggregates.Order.Order();
        dpTest.MockDpDomain(order);
        dpTest.Set<Guid>(order, "ID", FixedID);
...
#region add
    ...
    [Trait("Aggregate", "Add")]
    [Trait("Aggregate", "Fail")]
    public void Add_PromoCode_Missing_Fail()
    {
        //Arrange
        var dpTest = new DpTest();
        var order = Create_Order_With_PromoCode_Required_Property_Missing(dpTest);
        //Act and Assert
        var ex = Assert.Throws<PublicException>(order.Add);
        Assert.Equal("Public exception", ex.ErrorMessage);
        Assert.Collection(ex.Exceptions, i => Assert.Equal("PromoCode is required", i));
        Assert.False(order.Dp.Notifications.IsValid);
    }
...
#region update
    ...
    [Trait("Aggregate", "Update")]
    [Trait("Aggregate", "Fail")]
    public void Update_PromoCode_Missing_Fail()
    {
        //Arrange
        var dpTest = new DpTest();
        var order = Create_Order_With_PromoCode_Required_Property_Missing(dpTest);
