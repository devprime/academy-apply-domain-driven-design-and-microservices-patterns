...
public class Order
{
    ...
    public static Domain.Aggregates.Order.Order ToDomain(Application.Services.Order.Model.Order order)
    {
        ...
        Domain.Aggregates.Order.Order _order = new Domain.Aggregates.Order.Order(order.ID, order.CustomerTaxID, order.CustomerName, order.PromoCode, Application.Services.Order.Model.Item.ToDomain(order.Items), order.TotalPrice);
        return _order;
    }
