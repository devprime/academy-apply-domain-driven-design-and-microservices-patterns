...
public class Order
{
    ...
    public static Domain.Aggregates.Order.Order ToDomain(Application.Services.Order.Model.Order order)
    {
        ...
        Domain.Aggregates.Order.Order _order = new Domain.Aggregates.Order.Order(order.ID, order.CustomerName, order.CustomerEmail, Application.Services.Order.Model.Item.ToDomain(order.Items));
        return _order;
    }
