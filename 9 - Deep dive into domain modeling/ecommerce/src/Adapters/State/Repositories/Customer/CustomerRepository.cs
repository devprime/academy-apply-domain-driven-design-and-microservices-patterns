...
#region Read
    ...
    public Domain.Aggregates.Customer.Customer GetByEmail(string email)
    {
        return Dp.Pipeline(ExecuteResult: (stateContext) =>
        {
            var state = new ConnectionMongo(stateContext, Dp);
            var customer = state.Customer.Find(p => p.Email == email).FirstOrDefault();
            var _customer = ToDomain(customer);
            return _customer;
        });
    }
...
#region mappers
    ...
    public static Domain.Aggregates.Customer.Customer ToDomain(DevPrime.State.Repositories.Customer.Model.Customer customer)
    {
        if (customer is null)
            ...
            {IsNew = true};
        Domain.Aggregates.Customer.Customer _customer = new Domain.Aggregates.Customer.Customer(customer.ID, customer.Email, customer.Name);
        return _customer;
    }
