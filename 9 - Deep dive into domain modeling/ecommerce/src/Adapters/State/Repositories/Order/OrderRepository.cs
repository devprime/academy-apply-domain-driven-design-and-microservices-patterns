...
#region Read
    ...
    private Expression<Func<Model.Order, object>> GetOrdering(string field)
    {
        ...
        if (!string.IsNullOrWhiteSpace(field))
        {
            ...
            else if (field.ToLower() == "customeremail")
                exp = p => p.CustomerEmail;
            else
                exp = p => p.ID;
        }
        return exp;
    ...
    private FilterDefinition<Model.Order> GetFilter(string filter)
    {
        ...
        {
            var CustomerEmailFilter = builder.Eq(x => x.CustomerEmail, CustomerEmail);
            bfilter &= CustomerEmailFilter;
        }
        exp = bfilter;
        return exp;
    }
...
#region mappers
    ...
    public static Domain.Aggregates.Order.Order ToDomain(DevPrime.State.Repositories.Order.Model.Order order)
    {
        if (order is null)
            ...
            {IsNew = true};
        Domain.Aggregates.Order.Order _order = new Domain.Aggregates.Order.Order(order.ID, order.CustomerName, order.CustomerEmail, ToDomain(order.Items));
        return _order;
    }
    public static Domain.Aggregates.Order.Item ToDomain(DevPrime.State.Repositories.Order.Model.Item item)
    {
        if (item is null)
            ...
            {IsNew = true};
        Domain.Aggregates.Order.Item _item = new Domain.Aggregates.Order.Item(item.ID, item.Quantity, item.SKU, item.Price);
        return _item;
    }
